...
export abstract class BaseAjaxClient {
  ...
  protected GetHttpDataAsync = async <Req>(
    ...
    if (contentType !== "" && contentType !== "application/json") {
      throw new Error(
        "Content Type other then JSON not supported at the moment."
      );
    }
    if (headers == null) {
    ...
    let response = await this.FetchAsync(
      fullReqUrl,
      method,
      headers,
      reqBodyTxt
    ...
    if (response == null) {
      throw new Error(
        "Response null after api call. please report the event to administrator."
      );
    }
    return response;
  };
